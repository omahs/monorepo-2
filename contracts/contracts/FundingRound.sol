  ...
  InitialVoiceCreditProxy
{
  ...
  uint256 public precision = uint256(10)**18;
   ...
   */
  function setMaci(MACI _maci) external onlyOwner {
    require(
      address(maci) == address(0),
  ...
  function contribute(PubKey calldata pubKey, uint256 amount) external {
    ...
    require(
      contributorCount < maci.maxUsers(),
      'FundingRound: Contributor limit reached'
    );
    require(
      block.timestamp < maci.calcSignUpDeadline(),
      'FundingRound: Contribution period ended'
    );
    ...
    require(
      ...
      'FundingRound: Contribution amount is too large'
    );
    require(
      contributors[msg.sender].voiceCredits == 0,
      'FundingRound: Already contributed'
    );
  ...
  function register(
    ...
    require(
      msg.sender == address(maci),
      'FundingRound: Only MACI contract can register voters'
    );
    address user = abi.decode(_data, (address));
    bool verified = userRegistry.isVerifiedUser(user);
    require(verified, 'FundingRound: User has not been verified');
    require(
      contributors[user].voiceCredits > 0,
      'FundingRound: User has not contributed'
    );
    require(
    ...
    onlyOwner
  {
    ...
    require(address(maci) != address(0), 'FundingRound: MACI not deployed');
    require(
      maci.calcVotingDeadline() < block.timestamp,
      'FundingRound: Voting has not been finished'
    );
    require(
      !maci.hasUntalliedStateLeaves(),
      'FundingRound: Votes has not been tallied'
    );
    require(
      bytes(tallyHash).length != 0,
      'FundingRound: Tally hash has not been published'
    );
    ...
    budget = nativeToken.balanceOf(address(this));
    // alpha = (budget) / (sum of voice credits to all projects + sum of votes for each project) - 1
    alpha =
      ((budget * precision) / (totalSpent + maci.totalVotes())) -
      precision;
    isFinalized = true;
  }
